...
- **Роли:** расписаны два варианта — на 3 и на 4 человека.
...
Принцип: схема «инфраструктура / основная логика / тесты+презентация» из task.md. Код пишут все, но у каждого есть зона ответственности и финальный владелец.

### Вариант на 4 человека

| # | Роль | Зона ответственности |
| - | ---- | -------------------- |
| 1 | **Инфраструктура / Индексатор** | `db.py`, `models.py`, `indexer.py`, схема БД, `requirements.txt`, структура репо, `Dockerfile`. Владелец вопроса «почему такая схема». |
| 2 | **API / Backend** | `main.py`, `crud.py`, `schemas.py` — три основных эндпоинта, корректные коды (`[]` вместо 500, 404). |
| 3 | **Бонусы / QA** | `/api/stats`, `?type=`, пагинация; тесты `tests/` (особенно проверка пустого результата); прогон на 60-файловом датасете. |
| 4 | **Презентация / Docs / Release** | `README.md`, GitHub-репозиторий, сценарий 5-минутной демо, подготовка ответов на вопросы экспертов, проверка воспроизводимости «из чистой среды». |

### Вариант на 3 человека

Роли 1 и 4 частично объединяются; презентацию готовят совместно.

| # | Роль | Зона ответственности |
| - | ---- | -------------------- |
| 1 | **Индексатор + БД** | `db.py`, `models.py`, `indexer.py`, схема и индексы, `requirements.txt`, `Dockerfile`. Владелец вопроса про схему. |
| 2 | **API + бонусы** | `main.py`, `crud.py`, `schemas.py`, три эндпоинта + `/api/stats`, `?type=`, пагинация; коды ответов. |
| 3 | **QA + презентация + README** | тесты, прогон на датасете, `README.md`, GitHub, сценарий демо. |
